        ...
        private static String getChar(int x, int y){
            ...
            for (Person person: Main.allTeam) {
                if (person.coordinate_person.x== x && person.coordinate_person.y == y){
                    if (person.getHp() == 0) {
                        out = "|" + (AnsiColors.ANSI_RED + person.toString().charAt(0) + AnsiColors.ANSI_RESET);
                        break;
                    }
                    if (Main.team1.contains(person)) out = "|" + (AnsiColors.ANSI_GREEN + person.toString().charAt(0) + AnsiColors.ANSI_RESET);
                    if (Main.team2.contains(person)) out = "|" + (AnsiColors.ANSI_BLUE + person.toString().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
            }
            return out;
