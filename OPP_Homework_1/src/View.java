    ...
    private static String getChar(int x, int y) {
        ...
        for (Person person : Main.allTeam) {
            if (person.getCoords().get(0) == x && person.getCoords().get(1) == y) {
                ...
                if (Main.team1.contains(person))
                    out = "|" + (AnsiColors.ANSI_GREEN + person.toString().charAt(6) + AnsiColors.ANSI_RESET);
                if (Main.team2.contains(person))
                    out = "|" + (AnsiColors.ANSI_BLUE + person.toString().charAt(6) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
